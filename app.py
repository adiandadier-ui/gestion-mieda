...
def initialiser_fichiers_csv():
    ...
    if not os.path.exists(CSV_UTILISATEURS):
        df_init_users = pd.DataFrame({
            'Identifiant': ['admin', 'serveur1'],
            'Mot_De_Passe': ['admin123', 'pass123'],
    ...
    with tabs_caisse[0]:
        if df_actives.empty:
            ...
            def formater_libelle(row):
                if pd.notna(row['Accompagnement']) and row['Accompagnement'] != "-" and row['Accompagnement'] != "Sans":
            ...
            if col_btn1.button(f"Encaisser et Clôturer la {table_selectionnee} 💰", type="primary"):
                # CORRECTION ICI : Remplacement du masque erroné .index par un masque booléen correct
                masque = (st.session_state.historique_ventes['Table'] == table_selectionnee) & (st.session_state.historique_ventes['Statut'] == 'En cours')
                st.session_state.historique_ventes.loc[masque, 'Statut'] = 'Payé'
                sauvegarder_ventes()
            ...
            if col_btn2.button(f"Annuler l'addition de la {table_selectionnee} ❌"):
                # CORRECTION ICI : Correction du filtre booléen Pandas
                masque = (st.session_state.historique_ventes['Table'] == table_selectionnee) & (st.session_state.historique_ventes['Statut'] == 'En cours')
                st.session_state.historique_ventes.loc[masque, 'Statut'] = 'Annulé'
                sauvegarder_ventes()
...
def vue_configuration_carte():
    ...
    if action == "➕ Ajouter un Nouveau Produit":
        ...
        if not st.session_state.base_menu.empty:
            ...
            # CORRECTION ICI : Clôture propre du formulaire de modification qui était tronqué
            with st.form("form_edit"):
                ...
                if st.form_submit_button("✏️ Enregistrer les modifications"):
                    ...
                    st.session_state.base_menu.loc[idx, 'Prix_Vente_FCFA'] = edit_px_v
                    st.session_state.base_menu.loc[idx, 'Prix_Achat_Moyen_FCFA'] = edit_px_a
                    sauvegarder_menu()
...
def main():
    if not st.session_state.authentifie:
        ...
        # Barre latérale de navigation basée sur les privilèges du rôle
        ...
        # Routage des vues
        if choix == "📝 Prise de Commande":
            vue_prise_commande()
        elif choix == "🧾 Commandes & Additions":
            vue_commandes_additions()
        elif choix == "📦 Stocks & Approvisionnements":
            vue_stocks_appro()
        elif choix == "📊 Finances & Marges":
            vue_finances_marges()
        elif choix == "🔒 Clôture de Caisse":
            vue_cloture_caisse()
        elif choix == "⚙️ Configuration Carte":
