...
def vue_prise_commande():
    ...
    with col1:
        ...
        if not dict_menu:
            st.info("Aucun plat ou boisson commercialisable configuré pour le moment.")
            return
        ...
        with st.form("form_commande_strict", clear_on_submit=True):
            ...
            if st.form_submit_button("Envoyer la commande 🚀"):
                ...
                nom_matiere_brute, coef_defalquage = determiner_matiere_premiere(
                    item_details['Designation'], 
                    item_details['Prix_Vente_FCFA'],
                    item_details['Categorie']
                )
                ...
                if nom_matiere_brute:
                    ...
                    if not match_brute.empty:
                        ...
                        st.error(f"❌ Stock source introuvable : Impossible de trouver '{nom_matiere_brute}' en stock pour décalquer cette vente.")
                        st.stop()
                ...
                if quantite_a_deduire > target_details['Quantite_Dispo']:
                    ...
                    sauvegarder_ventes()
                    st.success(f"Commande envoyée ! Stock de réserve réduit de {quantite_a_deduire} sur '{matiere_a_deduire}'")
                    st.rerun()
